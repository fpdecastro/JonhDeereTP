...
if __name__ == "__main__":
    ...
    sql_query = ''' INSERT INTO hoursOfOperation(type_hours, startDate, endDate, engine_state, id_machine)
                     VALUES (?, ?, ?, ?, ?)'''
    ...
    for organizations in response_values:
        ...
        if response_machine_json_value != []:
            ...
            for machine in response_machine_json_value:
                ...
                url_newest = url1 + '/machines/' +id_machine + '/hoursOfOperation'
                print(url_newest)
                response_hours_json = request_test(url_newest, headers, AUTH)
                response_hours_value = response_hours_json['values']

                #Exiten máquienas que no tienen alertas
                if response_hours_value != []:

                    for hours in response_hours_value:
                        type_hours = hours['@type']
                        startDate = hours['startDate']
                        endDate = hours['endDate']
                        engineState = hours['engineState']
                        
                        insert_value = (type_hours, startDate, endDate, engineState, id_machine)
                        
                        cursor.execute(sql_query, insert_value)

                        print('Data was loaded succesfully: ', type_hours)
