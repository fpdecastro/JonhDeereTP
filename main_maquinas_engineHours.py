...
if __name__ == "__main__":
    ...
    sql_query = ''' INSERT INTO engineHours(engineHours_value, engineHours_unit, reportTime, id_machine)
                     VALUES (?, ?, ?, ?)'''
    ...
    for organizations in response_values:
        ...
        if response_machine_json_value != []:
            ...
            for machine in response_machine_json_value:
                ...
                url_newest = url1 + '/machines/' +id_machine + '/engineHours'
                print(url_newest)
                response_engine_json = request_test(url_newest, headers, AUTH)
                response_engine_value = response_engine_json['values']

                #Exiten máquienas que no tienen alertas
                if response_engine_value != []:

                    for hours in response_engine_value:
                        engineHours_value = hours['reading']['valueAsDouble']
                        engineHours_unit = hours['reading']['unit']
                        reportTime = hours['reportTime']
                        
                        insert_value = (engineHours_value, engineHours_unit, reportTime, id_machine)
                        
                        cursor.execute(sql_query, insert_value)

                        print('Data was loaded succesfully: ', engineHours_value)

                else:
                    print('Esta máquia no tiene hours of engine: ', id_machine)
